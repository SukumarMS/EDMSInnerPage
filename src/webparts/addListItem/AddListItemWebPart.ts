...
export default class AddListItemWebPart extends BaseClientSideWebPart<IAddListItemWebPartProps> {
  ...
  public AddItemPageLoad() {
    ...
    Addevent.addEventListener("click", (e: Event) => this.AddNewItem());

    // TRIGGER EVENT DELETE

    let Closeevent = document.getElementById('CloseItem');
    ...
    let videochangeEvent = document.getElementById('uploadVideoFileType');
    if (videochangeEvent) { videochangeEvent.addEventListener("change", (e: Event) => this.validateVideoFileType()); }

    // TRIGGER FILE VALIDATOR - IMAGE

    let changeEvent = document.getElementById('uploadImageFile');
    if (changeEvent) { changeEvent.addEventListener("change", (e: Event) => this.validateImageFileType()); }
  ...
  public VideoGallUpload() {
    ...
    if (isAllfield) {
      ...
      pnp.sp.web.lists.getByTitle("Video Gallery").rootFolder.folders.add($('#txtFolderName').val())
        .then(data => {
          if (radioValue == "Upload") {
            var files = <HTMLInputElement>document.getElementById("uploadVideoFileType");
            let file = files.files[0];
            var VideoTitle = { Title: $('#txtTitle').val().trim() };
            pnp.sp.web.getFolderByServerRelativeUrl("Video Gallery" + "/" + $('#txtFolderName').val()).files.add(file.name,
              file, true)
            ...
            if ($("#uploadVideoFileType").val() == undefined || $("#uploadVideoFileType").val() == null || $("#uploadVideoFileType").val() == '') {
              let rootSiteUrl = this.context.pageContext.site.absoluteUrl
              let siteTitle = rootSiteUrl.lastIndexOf("/") + 1;
              $.ajax({
                url: this.context.pageContext.site.absoluteUrl + "/_api/web/getfilebyserverrelativeurl('/sites/" + siteTitle + "/_catalogs/masterpage/Bloom/images/logo.png')/openbinarystream",
                type: "GET",
                success: function (data) {
                  var name = $("#txtTitle").val().trim() + '.jpg';
                  pnp.sp.web.getFolderByServerRelativeUrl("Video Gallery" + "/" + $('#txtFolderName').val()).files.add(name,
  ...
  public MediaImageUpload() {
    ...
    if (isAllfield) {
      ...
      pnp.sp.web.getFolderByServerRelativeUrl(strLocalStorage).files.add(uniquename, file, true).then(function (result) {
        result.file.listItemAllFields.get().then((listItemAllFields) => {
          pnp.sp.web.lists.getByTitle(strLocalStorage).items.getById(listItemAllFields.Id).update(myobjQl).then(function (result: any) {
            $('.addbutton').prop('disabled', true);
            window.history.back();
            $body.removeClass("loading");
          });
    ...
    if (isAllfield) {
      ...
      pnp.sp.web.getFolderByServerRelativeUrl("Media Gallery").files.add(uniquename, file, true)
        .then(function (result) {
          ...
          result.file.listItemAllFields.get().then((listItemAllFields) => {
            pnp.sp.web.lists.getByTitle(strLocalStorage).items.getById(listItemAllFields.Id).update(videoCols).then(function (result: any) {
              $body.removeClass("loading");
              $('.addbutton').prop('disabled', true);
              window.history.back();
            });
