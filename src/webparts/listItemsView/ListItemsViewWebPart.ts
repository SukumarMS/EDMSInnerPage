...
import styles from './ListItemsViewWebPart.module.scss';
import * as strings from 'ListItemsViewWebPartStrings';
import {readItems,addItems, updateItem, formatDate, checkUserinGroup, GetQueryStringParams, batchDelete} from '../commonJS';
import 'jquery';
require('jplist-core');
    ...
    if(strLocalStorage == "ProjectQuickLinks")
      ...
      if (this.userflag == false) {
          if (strLocalStorage == "Holiday") {
              ...
              objResults = await readItems(strLocalStorage, ["ID", "Title", "Modified", "Image", "Display"], count, "Modified", "Display", 1);
            ...
            objResults.then((items: any[]) => {
              ...
              objResults.then((items: any[]) => {
                ...
                objResults = await readItems(strLocalStorage, ["MediaFileType","ID", "Title", "Modified", "Display", "LinkURL","Image"], count, "Modified");
                this.renderhtml(objResults, strLocalStorage);
            }
